...
`guard` imports the `task` and `target` managers/guards, so the three must be installed together. The `pi` manifest in `package.json` loads all seven extension entry points (`extensions/*/index.ts`) in one `pi install npm:pi-atlas`; `shared/` is a helper module (not an entry) and `pi-acp-v2/` is a standalone stdio bin, so neither is an extension entry. Install only the subset you want by filtering in your `settings.json`.
